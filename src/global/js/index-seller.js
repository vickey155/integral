    ...
    var checkForm = function () {
        ...
        inputValidate.each(function(){
            ...
            if(selVal == ''){
                ...
                else if(selType == 'password' && !password.test(selVal)){
                    var errTip = "密码有6-16位数字，字母或特殊符号组成";
                    return showDialog(sel,dialogExist,errTip);
                }
                else if(selType == 'rePassword'){
                    var passwordVal = $.trim($('input[data-validate="password"]').val());
                    if(selVal != passwordVal){
                       var errTip = "两次输入的密码不一致";
                       return showDialog(sel,dialogExist,errTip);
                   }
                }
            }
